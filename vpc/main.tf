...
locals {
  ...
  nat_gateway_count  = var.single_nat_gateway ? toset(local.availability_zones[0]) : var.one_nat_gateway_per_az ? toset(local.availability_zones) : {}
}

resource "aws_eip" "this" {
  for_each = var.create_vpc && var.create_nat_gw ? local.nat_gateway_count : {}
