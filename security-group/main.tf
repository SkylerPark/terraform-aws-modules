...
resource "aws_vpc_security_group_ingress_rule" "ingress_with_self" {
  for_each = { for rule in var.ingress_with_self : format("%s_%s_%d_%d_%s", "ingress", rule.ip_protocol, rule.from_port, rule.to_port, "self") => rule }
...
resource "aws_vpc_security_group_egress_rule" "egress_with_self" {
  for_each = { for rule in var.egress_with_self : format("%s_%s_%d_%d_%s", "egress", rule.ip_protocol, rule.from_port, rule.to_port, "self") => rule }
